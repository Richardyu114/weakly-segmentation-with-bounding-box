...
def test_model():
    ...
    model = torch.load('model_epoch10.pth')
...
def miou_evaluation():
    mIoU,IoU = metric.mIoU_with_GT(is_val=False)
    print('(the test result is) mIoU: {}, IoU: {}'.format
                             ('%.3f' %mIoU, np.around(IoU, decimals=3)))
# def miou_evaluation():
